...
public class CharacterSelectionController : MonoBehaviour {
    ...
    private float IdleRotationSpeed = 25f;
    ...
    void Update()
    {
        ...
        if (Input.GetButtonDown(PlayerInputPrefix + "MenuCycleButtonOne"))
            ChangeSelectedPlatformCharacterSkin();
        ...
        for (int i = 0; i < CharacterPlatforms.Count; i++)
        {
            if (i != m_SelectedPlatformIndex)
            {
                ...
                l_CharacterPlatform.GetComponent<PlatformController>().ResetCharacterSkin();
            }
        }
    ...
    void RotateSelectedPlatform(float p_RotationValue)
    {
        ...
        l_SelectedCharacterPlatform.transform.Rotate(Vector3.up, p_RotationValue * Time.deltaTime);
    }
    ...
    public void ChangeSelectedPlatformCharacterSkin()
    {
        GameObject l_SelectedCharacterPlatform = CharacterPlatforms[m_SelectedPlatformIndex];

        l_SelectedCharacterPlatform.GetComponent<PlatformController>().ChangeCharacterSkin();
    }
    ...
    public void ResetSelectedPlatformRacesuitSkin()
    {
        ...
        l_SelectedCharacterPlatform.GetComponent<PlatformController>().ResetCharacterSkin();
